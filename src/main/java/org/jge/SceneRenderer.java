...
public class SceneRenderer {
    ...
    public void render(Scene scene) throws Exception {
        ...
        rebuild = scene.rebuildLightMap;
        
        boolean calc = scene.calculateLightMap;

        if(scene.lightMapFile == null) {
            rebuild = false;
            calc = false;
        } else if(!scene.lightMapFile.exists()) {
            rebuild = true;
        }
            ...
            try {
                ...
                for(Node node : meshes) {
                    ...
                    for(int[] polygon : mesh.polygons) {
                        if(polygon.length == 4) {
                            VertexPT2N v1 =  mesh.vertices.get(polygon[0]);
                            VertexPT2N v2 =  mesh.vertices.get(polygon[1]);
                            VertexPT2N v3 =  mesh.vertices.get(polygon[2]);
                            VertexPT2N v4 =  mesh.vertices.get(polygon[3]);
                            ...
                            for(int i : polygon) {
                                VertexPT2N v = mesh.vertices.get(i);
                ...
                if(rebuild) {
                    ...
                    ImageIO.write(image, "PNG", scene.lightMapFile);
                }

                Game.getInstance().assets.unload(scene.lightMapFile);

                Texture texture = Game.getInstance().assets.load(scene.lightMapFile);
