...
public class Node implements Iterable<Node> {
    ...
    public Node(Node node, boolean deepCopy) throws Exception {
        if(node.renderable != null) {
            renderable = node.renderable.newInstance(deepCopy);
        }
        ...
        lightMapColor.set(node.lightMapColor);
        overrideSceneLightMapColor = node.overrideSceneLightMapColor;
        zOrder = node.zOrder;
        lightMapEnabled = node.lightMapEnabled;
        depthState = node.depthState;
        cullState = node.cullState;
        blendState = node.blendState;
        minTrisPerTree = node.minTrisPerTree;
        triangleTag = node.triangleTag;
        ...
        for(Node child : node) {
            addChild(new Node(child, deepCopy));
        }
    }
